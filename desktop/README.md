...
- 桌面端从资源管理器拖文件到输入框：通过 Tauri 原生拖拽引用原路径，只查询文件元信息，不读取内容、不上传、不复制到工作区。图片也作为文件路径引用，不自动转为模型图片输入；需要看图时由工具读取原图。浏览器拖拽仍走上传；选择附件按钮和剪贴板图片保持原行为。
- 原生事件只交给当前受管运行时；仅在输入框区域落下时接收附件，支持 DPI/页面缩放坐标换算。目录、UNC 路径、失效文件会报错，不回退上传。隔离模式的工作区权限检查仍生效。文件移动或删除后引用会失效，修改原文件后后续工具读取的是修改后的内容。
