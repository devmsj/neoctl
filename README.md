...
├── desktop/  # Windows 桌面端、本地运行时安装与启动管理
...
Desktop 安装、构建及本地后台启动管理说明见 [`desktop/README.md`](desktop/README.md)。模型配置由用户在本地应用中管理，会话保存在本地数据目录。
...
测试文件统一收纳到各模块的 `tests/` 目录。目录规范、执行入口及环境要求见 [`TESTING.md`](TESTING.md)。
