...
const fixture=await createObservabilityBrowser(()=>snapshot,async route=>{
  ...
  if(!pathname.startsWith('/api/obs10-') && !pathname.startsWith('/api/images/')) return false
...
try {
  ...
  // Reproduce the production shape: result line + a sibling image with only src.
  const legacy=line(3,'auto',[])
  legacy.text='generated 1 image\nrequested: openai · gpt-image-2.5-sunburst · auto · png\nupstream-reported quality: medium'
  legacy.text+='\nwarning [OPTIONAL_CHECK]: Optional metadata verification failed; image retained.'
  legacy.imageResult={model:'gpt-image-2.5-sunburst',size:'auto',quality:'auto',images:[{label:'legacy-mug',width:31,height:17,hasAlphaChannel:true,hasTransparentPixels:false}]}
  const sibling={id:4,kind:'tool',parentToolName:'image_create',parentToolUseId:'call-3',messageId:'message-3',text:'legacy-mug',image:{src:'/api/images/by-id/img_legacy',imageId:'img_legacy',label:'legacy-mug',mimeType:'image/png',available:true,blockIndex:1}}
  snapshot=observabilitySnapshot({lines:[main,auto,legacy,sibling]})
  ...
  await page.keyboard.press('Escape')
  assert.equal(await page.locator('.image2-result-shell').nth(2).locator('.image2-summary strong').innerText(),'生成完成')
  await open(2);await page.getByText('31 × 17',{exact:true}).waitFor()
  assert.match(await rows().first().innerText(),/Alpha 通道\s+有[\s\S]*透明像素\s+无/i)
  assert.match(await rows().first().innerText(),/auto（自动策略）/)
  assert.equal(fixture.requests.some(r=>r.url.includes('/api/images/by-id/img_legacy')),true)
  await page.keyboard.press('Escape')
