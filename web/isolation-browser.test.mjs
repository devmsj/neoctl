...
const passwordHash = await hashPassword('BrowserTestOnly');
...
try {
  ...
  await page.getByLabel('密码', { exact: true }).fill('BrowserTestOnly');
  await page.getByRole('button', { name: '登录', exact: true }).click();
  ...
  assert.equal(await page.getByLabel('密码', { exact: true }).getAttribute('maxlength'), null);
  await page.getByRole('button', { name: '登录', exact: true }).click();
  await page.getByRole('button', { name: '退出登录', exact: true }).waitFor();
  await page.getByRole('button', { name: '退出登录', exact: true }).click();
  await page.getByLabel('用户名', { exact: true }).fill('Charlie');
  await page.getByLabel('密码', { exact: true }).fill('2');
  await page.getByRole('button', { name: '登录', exact: true }).click();
  await page.getByRole('alert').waitFor();
  await page.getByLabel('密码', { exact: true }).fill('1');
  await page.getByRole('button', { name: '登录', exact: true }).click();
