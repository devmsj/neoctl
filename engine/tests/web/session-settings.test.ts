...
import type { CompactionReport } from "../../src/context/compaction.js";
import type { QueryEngine } from "../../src/core/query-engine.js";
import { WebRepl, type WebRuntime } from "../../src/web/index.js";
...
// Run from engine/: node --import tsx --test tests/web/session-settings.test.ts
...
test("HTTP source contract: POST routes pass JSON fields directly to session-only methods", () => {
  ...
  const source = fs.readFileSync(new URL("../../src/web/index.ts", import.meta.url), "utf8");
  const routes = [
