...
import { PromptConfigStore, PromptConfigError, MAX_SYSTEM_PROMPT_BYTES, readBundledSystemPrompt } from "../../src/context/prompt-config.js";
import { AdditionalPromptContextManager, AppPromptContextManager, DefaultContextManager } from "../../src/context/context-manager.js";
import { InMemoryAppPromptStore } from "../../src/app/app-prompt.js";
...
test("atomic file survives a separate process and cross-process CAS admits only one writer", async (t) => {
  const { store, filePath } = await fixture(t);
  const initial = await store.read();
  const moduleUrl = new URL("../../src/context/prompt-config.js", import.meta.url).href.replace(/\.js$/, import.meta.url.endsWith(".ts") ? ".ts" : ".js");
