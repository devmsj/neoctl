...
export const IMAGE_EDIT_REFERENCE_GUIDE = "For edits, use the exact registry ID shown for the source (for example img_1) in imageRefs; never copy a caption, URI, storage path, or surrounding text. If the intended source is simply the latest image, omit imageRefs and keep useLatestImage=true.";
...
export const IMAGE_INPUT_SCHEMA: JsonSchema = {
  ...
  properties: {
    ...
    imageRefs: { type: "array", minItems: 1, maxItems: 16, items: { type: "string" }, description: "Exact prior-image registry IDs (for example img_1); labels/numeric refs remain compatible. Never pass captions, URIs, storage paths, or surrounding text. Unknown/ambiguous refs fail rather than selecting another image." },
